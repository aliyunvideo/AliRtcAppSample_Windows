...
void CRTCSampleMainInterface::OnBnClickedCheckCameraMirror()
{
	...
	canvas.hWnd = GetDlgItem(m_vecThrowingScreenInfo[m_mapRemoteInfo[m_vecRemoteInfoList.at(0).c_str()].s_iReflectIndex].s_iReflectID)->m_hWnd;
...
void CRTCSampleMainInterface::OnBnClickedCheckScreenMirror()
{
	...
	AliVideoCanvas canvas;
	canvas.hWnd = GetDlgItem(m_vecThrowingScreenInfo[m_mapRemoteInfo[m_vecRemoteInfoList.at(0).c_str()].s_iReflectIndex + 1].s_iReflectID)->m_hWnd;
	canvas.flip = bIsCheck;
...
void CRTCSampleMainInterface::onRemoteUserOnLineNotify(const AliRtc::String & uid)
{
	...
	WRILOG(LOGTYPE_DEBUG, "DEBUG3.LOG", "onRemoteUserOnLineNotify", "2远端入会[uid:%s][在线人数：%d]", uid.c_str(), m_mapRemoteInfo.size());
	ShowNumOfChannels();
...
LRESULT CRTCSampleMainInterface::onRemoteUserOffLineNotifyMT(WPARAM wParam, LPARAM lParam)
{
	...
	if (m_vecRemoteInfoList.size() > 5)
	{
		if (m_iRightSideIndex == m_vecRemoteInfoList.size())
		{
			...
			ShowSubscribe(m_vecRemoteInfoList[m_iLeftSideIndex], 0, AliRtcVideoTrackCamera);
		...
		else
		{
			...
			ShowSubscribe(m_vecRemoteInfoList[m_iRightSideIndex], 4, AliRtcVideoTrackCamera);
	...
	{
		RemoveRemoteUser(RemoteUserOffLine->uid, BackwardShift);
		ShowSubscribe(m_vecRemoteInfoList[4], 4, AliRtcVideoTrackCamera);
...
LRESULT CRTCSampleMainInterface::onRemoteTrackAvailableNotifyMT(WPARAM wParam, LPARAM lParam)
{
	...
	vector<AliRtc::String>::iterator VecIterCamera = m_vecRemoteInfoList.end();

	if (0 != m_pEngine->getUserInfo(subChange->uid, dict))  goto exit;
	if (0 == m_mapRemoteInfo.count(subChange->uid.c_str())) goto exit;
	
	for (vector<AliRtc::String>::iterator VecIter = m_vecRemoteInfoList.begin(); VecIter != m_vecRemoteInfoList.end(); VecIter++)
	{
		if (*VecIter == subChange->uid)
		{
			...
			{
				if (AliRtcVideoTrack::AliRtcVideoTrackBoth != subChange->vt)
				{
	...
	if (AliRtcVideoTrack::AliRtcVideoTrackBoth == subChange->vt && !bIsScreen)
	{
...
void CRTCSampleMainInterface::ShowSubscribe(const AliRtc::String uid, int iReflectIndex, AliRtcVideoTrack vt)
{
	...
	else if (AliRtcVideoTrackScreen == vt)
	{
		if (true == m_mapRemoteInfo[uid.c_str()].s_bHsaScreenFlow && true == m_mapRemoteInfo[uid.c_str()].s_bIsRecvScreen)
		{
			AliVideoCanvas canvas;

			canvas.hWnd = GetDlgItem(m_vecThrowingScreenInfo[iReflectIndex].s_iReflectID)->m_hWnd;
...
bool CRTCSampleMainInterface::JoinChannel()
{
	...
	AliRtcAuthInfo authinfo;//备注：需要手动输入鉴权信息 https://help.aliyun.com/document_detail/146833.html?spm=5176.11065259.1996646101.searchclickresult.230e1c95nxlEcb#title-t41-agh-m2i
...
void CRTCSampleMainInterface::ClearChannel(const AliRtc::String uid, AliRtcVideoTrack videoTrack)
{
	if (0 == m_mapRemoteInfo.count(uid.c_str())) return;

	AliVideoCanvas canvas;
	canvas.hWnd = NULL;
	if (AliRtcVideoTrackCamera == videoTrack)
	{
		m_pEngine->setRemoteViewConfig(&canvas, uid, AliRtcVideoTrack::AliRtcVideoTrackCamera);
	...
	m_pEngine->subscribe(uid, onSubResultOld, this);
}
...
void CRTCSampleMainInterface::ReplaceSubscribeFlow(const AliRtc::String strOldUid, AliRtcVideoTrack oldvt, const AliRtc::String strNewUid, AliRtcVideoTrack newvt, int index)
{
	m_bMoving = false;
	if (!strOldUid.isEmpty())
	{
		AliVideoCanvas canvas;
		canvas.hWnd = NULL;
		if (AliRtcVideoTrackCamera == oldvt)
		{
			m_pEngine->setRemoteViewConfig(&canvas, strOldUid, AliRtcVideoTrack::AliRtcVideoTrackCamera);
			m_pEngine->configRemoteCameraTrack(strOldUid, m_mapRemoteInfo[strOldUid.c_str()].s_bBigStream, false);
		}
		else
		{
			m_pEngine->setRemoteViewConfig(&canvas, strOldUid, AliRtcVideoTrack::AliRtcVideoTrackScreen);
	...
	if (AliRtcVideoTrackCamera == newvt)
	{
		m_pEngine->configRemoteCameraTrack(strNewUid, m_mapRemoteInfo[strNewUid.c_str()].s_bBigStream, true);
		ShowSubscribe(strNewUid, m_iChangeChannelIndex, AliRtcVideoTrackCamera);
	}
	else if (true == m_mapRemoteInfo[strNewUid.c_str()].s_bHsaScreenFlow && true == m_mapRemoteInfo[strNewUid.c_str()].s_bIsRecvScreen)
	{
		m_pEngine->configRemoteScreenTrack(strNewUid, true);
		ShowSubscribe(strNewUid, m_iChangeChannelIndex, AliRtcVideoTrackScreen);
	}
	else
	{
	...
	m_pEngine->subscribe(strNewUid, onSubResultNew, this);
}
...
void CRTCSampleMainInterface::LeftShift(int iLeftSideIndex, int iRightSideIndex)
{
	...
	{
		ReplaceSubscribeFlow(m_vecThrowingScreenInfo[0].s_strRemoteUserUid, m_vecThrowingScreenInfo[0].vt, m_vecRemoteInfoList[iLeftSideIndex], AliRtcVideoTrackCamera, 0);
	}
}
