...
struct sRemoteInfo
{
	AliRtc::String s_strRemoteUserUid;
	AliRtc::String s_strRemoteUserName;
...
struct sUserInfo
{
	AliRtc::String s_strUserID;
	AliRtc::String s_strUserName;
...
{
	AliRtc::String s_strSetOccupyUid;
	bool           s_bIsRecVideo;
...
class CRTCSampleMainInterface : public CDialogEx, AliRtcEventListener
{
...
public:
	...
	void UpdateSubscribe(const AliRtc::String strUid, AliRtcAudioTrack audioTrack,
	    AliRtcVideoTrack videoTrack);                                                               // 更新订阅流(Update subscription stream)
	void ShowSubscribe(const AliRtc::String uid, int iReflectIndex, AliRtcVideoTrack vt);           // 投映订阅流(Reflect subscription stream)
	void ClearChannel(const AliRtc::String uid, AliRtcVideoTrack videoTrack);                                                    // 清空频道（Clear reflect）
	void ReplaceSubscribeFlow(const AliRtc::String strOldUid, AliRtcVideoTrack oldvt, 
		const AliRtc::String strNewUid, AliRtcVideoTrack newvt, int index);                         // 更换订阅流(Close subscription data)
	void ShowNumOfChannels();                                                                       // 展示频道在线人数(Number of Display Channels)
	void RemoveRemoteUser(const AliRtc::String strUid, RemoveType remove);                          // 移除离会用户（Remove Departure Users）
	...
	void RemoteUserPreferMasterSwitching(AliRtc::Dictionary &dict);                                 // 远程用户大小流切换（Remote User Large and Small Stream Switching）
	void RemoteUserMirrorSwitching(AliRtc::Dictionary &dict);                                       // 远程用户镜像切换（Remote User Mirror Switching）
	...
	AliRtc::String CStringToAliString(CString &s);                                                  // CString转AliString（CString To AliString）
	...
	void OnJoinChannelResult(int error);                                                            // 加入频道回调(Join channel function callback)
    ...
    virtual void onExternalDeviceStateChange(const AliRtc::String &deviceName, 
        AliRtcExternalDeviceState state) override;                                                  // 外接设备状态变更(State Change of External Equipment)
	...
	AliRtcEngine *m_pEngine;                                                                        // RTC SDK类

	UINT m_nConsoleOccupancy;                                                                       // 控制台占用

	map<CString, sRemoteInfo> m_mapRemoteInfo;                                                      // 远端用户信息
	vector<AliRtc::String> m_vecRemoteInfoList;                                                     // 用户流队列
